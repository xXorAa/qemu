...
{
    error_report("SECURE EC WRITE to %lu,%lu", offset, value);

    if (offset == 0) /* PSCI machine powerstate register */
    {
        switch(value) {
...
static void secure_ec_init(Object *obj)
{
    ...
    error_report("SECURE EC INIT");
    memory_region_init_io(&s->iomem, obj, &secure_ec_ops, s, "secure-ec", 0x1000);
...
static VMStateDescription vmstate_sbsa_secure_ec_info = {
    .name = "sbsa-secure-ec",
    .version_id = 0,
...
static void secure_ec_class_init(ObjectClass *klass, void *data)
{
    ...
    dc->vmsd = &vmstate_sbsa_secure_ec_info;
